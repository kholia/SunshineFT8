...
int decode_ft8(message_info message_list[])
{
  ...
  for (int idx = 0; idx < num_candidates; ++idx)
  {
    ...
    if (found_empty_slot)
    {
      ...
      /* Queue up results for spotting */
      if (!found_duplicate) {
        struct decoder_results dr;
        /* How many parts do we have in message.text */
        ...
        // __android_log_print(ANDROID_LOG_INFO, "<CLI>", "Decoded spaces is: %d\n", spaces);
        char *token = strtok(message.text, " ");
        for (int i = 0; i < spaces - 1; i++) {
          token = strtok(NULL, " ");
        }
        // Imperfect dupe removal
        if (token != NULL) {
          if (seen.find(token) == seen.end()) {
            strncpy(dr.call, token, strlen(token));
            // dr.freq = reporter->frequency + freq_hz;
            dr.freq = 21074000 + freq_hz; // XXX
            dr.snr = cand->score - 20;
